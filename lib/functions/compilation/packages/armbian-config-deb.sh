...
compile_armbian-config() {
	...
	# Linking
	#ln -sf /bin/armbian-config "${tmp_dir}/${armbian_config_dir}"/usr/bin/armbian-config

	# Add development repository to keep rooling release of this tool
	echo "deb [signed-by=/usr/share/keyrings/armbian.gpg] https://armbian.github.io/configng stable main" | sudo tee ${tmp_dir}/${armbian_config_dir}/etc/apt/sources.list.d/armbian-config.list > /dev/null
